...
站内表单只对登录成员开放，所以**「我登不进去」这类反馈这一版收不到** —— 尤其邀请码门槛合入之后，会多出一整类"码不对 / 码过期"的失败，而这些人永远看不到反馈入口。

这是明确接受的取舍，不是遗漏。两条本来可以补的路都不划算：给 `anon` 角色开 insert 权限等于开一个任何人都能匿名往库里灌字符串的公开写入端点；登录页挂 `mailto:` 则要把一个地址公开写进 HTML，会被爬虫抓走。

所以这一版就靠**邀请链接的分发渠道兜底** —— 链接是通过领航内部渠道一个个发出去的，登不进来的人本来就有一条人对人的路可以说。等邀请码门槛上线后真的出现一批人卡在门口，再补一个对外地址。
...
2. **不做任何"页面上下文"的自动采集**。技术上很容易在提交时顺手把"用户当前展开了哪些帖子"带上去 —— 一旦这么做，一个匿名帖作者随手提一句反馈，就在 `feedback` 表里留下了「这个身份 ↔ 这些帖子」的关联。这条链路必须从设计上不存在，而不是靠"我们不会去看"。所以：**只提交用户自己敲进去的文字**，连 `user_agent` 和页面路径都不带。
...
登录页不动，所以和 `feature/invite-code-gate` 不冲突。

> ⚠️ 与 `feature/notification-center` 会冲突：铃铛也加在 `.account-menu` 里。是几行的手工冲突，但**后合的那个要复查 topbar 的最终顺序**：期望是 `铃铛 · 反馈 · 账号名 · 退出`，别让反馈按钮挤到铃铛和名字中间。
...
| 5 | README 加一节「怎么读反馈」（一条 dashboard SQL）+ `product_design_summary.md` 进度段补一行 | 可合并 |
...
**A. 不带内容举报。** 只有一个通用反馈入口；想举报的人写在正文里，团队照样看得到。
现在是几十人的邀请制熟人社区，先看看真的有没有需要举报的东西，再决定要不要为它建结构 —— 举报要在每张卡片上加入口、在表上加 `post_id` / `reply_id`，而后者正撞上第 2 节那条"不要结构化地把身份和帖子关联起来"。

**B. 不做登录页的对外邮箱回退。** 这一版只有站内表单，代价是收不到"登不进去"这类反馈（见第 1 节末）。
不公开地址就不用承担爬虫和垃圾邮件，登录失败的人先靠邀请链接的分发渠道人对人反馈。等邀请码门槛上线后真的出现一批人卡在门口，再单独补。

**C. 不附带任何浏览器信息。** 只提交用户写的文字，`user_agent` 和页面路径都不采。
`can_contact` 已经给了追问的路，代价只是排查 bug 时可能得回头问一句"你用的什么浏览器"。真到了"反馈里一半是查不下去的 bug"那天再加，并且加的时候要在弹层里明写「会一起发送这些信息」。

---

## 7. 一句话总结

一张只能写不能读的 `feedback` 表 + topbar 上一个中性的「反馈」按钮 + 一个原生 `<dialog>`，零新增依赖、零新增密钥、零新增账单，半天到一天。
